...
func (what *Threagile) Init(buildTimestamp string) *Threagile {
	...
	return what.initRoot().initImport().initAnalyze().initCreate().initExecute().initExplain().initList().initPrint().initQuit().initServer().initVersion().initAIGenerate().processSystemArgs(what.rootCmd)
}
